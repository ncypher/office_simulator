...
5. **Drop in a situation** selects a preset or introduces your own event. Relationships and the transcript carry forward until you start a fresh session.
...
- Demo lines are explicitly scripted. Warmth and assertiveness affect delivery, and recent human speech is acknowledged. Arbitrary personality descriptions require Live AI for interpretation. Scripts eventually repeat.
- Trust is a simple game mechanic: support +3, challenge −2, deflection −1, neutral 0 for listeners toward the speaker. It is not a psychological measurement. Human lines are neutral because their intent is not automatically classified.
...
- State lasts for the current Streamlit session; a refresh/reconnect may lose it. Download the session to retain a readable JSON record. Import/resume is not implemented.
